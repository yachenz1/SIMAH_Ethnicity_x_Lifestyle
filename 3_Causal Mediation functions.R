...
CMed_race3_prep <- function(data) {
  ...
  mydata <- data %>%
    ...
    dplyr::select(A.race, M1.alc, M2.smk, M3.bmi, M4.phy, allcause_death, bl_age, end_age, married2, edu3, srvy_yr) %>%
    filter(complete.cases(A.race, M1.alc, M2.smk, M3.bmi, M4.phy, allcause_death, bl_age, end_age, married2, edu3, srvy_yr))
  ...
  fitM1 <- vglm(M1.alc ~ ATemp + bl_age + married2 + edu3 + srvy_yr, data = mydata, family=multinomial(refLevel = 3))
  fitM2 <- vglm(M2.smk ~ ATemp + bl_age + married2 + edu3 + srvy_yr, data = mydata, family=multinomial(refLevel = 1))
  fitM3 <- vglm(M3.bmi ~ ATemp + bl_age + married2 + edu3 + srvy_yr, data = mydata, family=multinomial(refLevel = 2))
  fitM4 <- vglm(M4.phy ~ ATemp + bl_age + married2 + edu3 + srvy_yr, data = mydata, family=multinomial(refLevel = 3))
  ...
  newMyData <- newMyData %>%
    dplyr::select(ID, bl_age, end_age, allcause_death, A.race, race_M1.alc, race_M2.smk, race_M3.bmi, race_M4.phy, married2, edu3, srvy_yr, weightM) %>%
    filter(complete.cases(ID, bl_age, end_age, allcause_death, A.race, race_M1.alc, race_M2.smk, race_M3.bmi, race_M4.phy, married2, edu3, srvy_yr, weightM))
