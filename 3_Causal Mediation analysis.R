...
output <- "~/Dropbox/Mac/Desktop/Public Health Institute/Alcohol Research Group/SIMAH/SIMAH_workplace/nhis/Restricted access data/Causal mediation/Race x Lifestyle/CausMed/18-85 years old/"
...
CMed_f <- aalen(Surv(bl_age, end_age, allcause_death) ~ const(A.race) * const(race_M1.alc) + 
                                                        ...
                                                        const(married.factor) + edu.factor + const(srvy_yr),  # adjust for survey year as continuous variable 
...
CMed_m <- aalen(Surv(bl_age, end_age, allcause_death) ~ const(A.race) * const(race_M1.alc) +
                                                        ...
                                                        const(A.race) * const(race_M3.bmi) +
                                                        const(A.race) * const(race_M4.phy) +
                                                        const(married.factor) + edu.factor + const(srvy_yr),  
                          data=expandedData, weights=expandedData$weightM, clusters=expandedData$ID, robust=0)
